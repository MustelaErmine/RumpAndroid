    ...
    public class iOSPostprocessUtils : MonoBehaviour
    {
        ...
        private static void updatePod(BuildTarget target, string buildPath)
        {
            ...
            using (var sw = File.AppendText(buildPath + "/Podfile"))
            {
                ...
                sw.Log("  project = installer.pods_project");
                sw.Log("  project.targets.each do |target|");
                sw.Log("    target.build_configurations.each do |config|");
                sw.Log("      config.build_settings['ENABLE_BITCODE'] = 'YES'");
                sw.Log("    end");
                sw.Log("  end");
                sw.Log("end");
            }
