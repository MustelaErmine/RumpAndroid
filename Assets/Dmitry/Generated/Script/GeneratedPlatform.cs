...
public class GeneratedPlatform : MonoBehaviour
{
    ...
    public float OverlapPlatforCount;
    ...
    public void OnDestroyAndCreate()
    {
        ...
        /*if (OverlapPlatforCount == LengeCreate - 2)
        {
            ...
            {
                GeneratedNewPlatform(Finish);
            }
            else {
                if (platformDestroy == FinishGenerateIndex / 3)
                {
                    GeneratedNewPlatform(Step1);
                }
                else if (platformDestroy >= (FinishGenerateIndex / 3) + 1 && platformDestroy <= FinishGenerateIndex - (FinishGenerateIndex / 3))
                {
                    ...
                    GeneratedNewPlatform(PlatformStep2[RandomPlatform]);
                ...
                else
                {
                    ...
                    if (platformDestroy > FinishGenerateIndex - (FinishGenerateIndex / 3))
                    {
                        if (DoOnce)
                        {
                            ...
                            GeneratedNewPlatform(ScatePlatform);
                            
                        }
                        else
                        {
                            int RandomPlatform = Random.Range(0, PlatformStep3.Length);
                            GeneratedNewPlatform(PlatformStep3[RandomPlatform]);
                            Debug.Log("generated");
                        }
                    }
                    else
                    {
                        ...
                        GeneratedNewPlatform(Platform[RandomPlatform]);
                    }
                }
            }
        }
    ...
    void GeneratedNewPlatform(GameObject platform)
    {
